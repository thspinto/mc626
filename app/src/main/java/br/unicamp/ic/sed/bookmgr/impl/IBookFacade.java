...
import br.unicamp.ic.sed.bookmgr.prov.IBook;
 ...
 * @author petero
 */
class IBookFacade implements IBook{
    ...
    public IBookFacade(boolean verbose) {
        this.verbose = verbose;

		// should not initialize at startup, it has a lot of cost
		// initBook();
    }

    /**
     * inits the book based on the inputstream
     * */
    private final void initBook(InputStream inStream) {
...
//            InputStream inStream = openBookStream("/book.bin");
...
//            inStream.close();
            ...
            for (int i = 0; i < len; i += 2) {
                ...
                if (move == 0) {
                    ...
                    Move m = new Move(move & 63, (move >> 6) & 63,
                                      promToPiece(prom, pos.isWhiteTurn() ));
                    if (!bad)
                        addToBook(pos, m);
                    pos.makeMove(m, ui);
                }
    ...
    public final Move getBookMove(Position pos) {
    	Log.w("debug", "getBookMove");

        initBook(null);

        List<BookEntry> bookMoves = bookMap.get(pos.zobristHash());
        if (bookMoves == null) {
        	Log.w("debug", "book moves null");
            return null;
        }

        Log.w("debug","bookMoves count=" +bookMoves.size());
        ...
        for (int i = 0; i < bookMoves.size(); i++) {
            ...
            for (int mi = 0; mi < legalMoves.size; mi++)
                if (legalMoves.m[mi].equals(be.move)) {
                	Log.w("debug", "within getBookMove book entry from:"+be.move.from
                			+ " to:" + be.move.to);
                    contains = true;
                    break;
                }
        ...
        Log.w("debug", "rnd value=" + rnd);
        sum = 0;
        for (int i = 0; i < bookMoves.size(); i++) {
            sum += getWeight(bookMoves.get(i).count);
            if (rnd < sum) {
            	Move m = bookMoves.get(i).move;
            	Log.w("debug", "entry returned book from:"+ m.from
            			+ " to:" + m.to);
                return m;
            }
        }
