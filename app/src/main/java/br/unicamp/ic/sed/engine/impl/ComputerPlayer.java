...
import br.unicamp.ic.sed.engine.req.IBook;
...
class ComputerPlayer implements Player {
    ...
    public ComputerPlayer(IManager mgr) {
    	this.manager = mgr;
    ...
    public String getCommand(Position pos, boolean drawOffer, List<Position> history) {
        ...
        if (bookEnabled) {
        	Log.w("debug", "within ComputerPlayer getBookMove");
        	IBook book = (IBook) this.manager.getRequiredInterface("IBook");
            //component Book is optional.
        	if (book != null) {
        		Log.w("debug", "before actual book.getBookMove");
	        	Move bookMove = book.getBookMove(pos);
	            if (bookMove != null) {
	                //System.out.printf("Book moves: %s\n", book.getAllBookMoves(pos));
	            	Log.w("debug", "Book moves not null on ComputerPlayer");
	                return TextIO.moveToString(pos, bookMove, false);
	            }
        	} else {
        		Log.w("debug", "Component IBook not found!, its a optional component");
        	}
        }
