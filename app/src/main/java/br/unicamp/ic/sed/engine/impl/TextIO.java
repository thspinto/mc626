...
class TextIO {
    ...
    public static final Position readFEN(String fen) throws ChessParseError {
        ...
        try {
            if (words.length > 4) {
                pos.setHalfMoveClock( Integer.parseInt(words[4]) );
            }
            if (words.length > 5) {
                pos.setFullMoveCounter( Integer.parseInt(words[5]) );
            }
        ...
        pos2.setWhiteMove(!pos.isWhiteTurn());
    ...
    public static final void fixupEPSquare(Position pos) {
        ...
        if (epSquare >= 0) {
            ...
            for (int mi = 0; mi < moves.size; mi++) {
                Move m = moves.m[mi];
                if (m.to == epSquare) {
                    if (pos.getPiece(m.from) == (pos.isWhiteTurn() ? Piece.WPAWN : Piece.BPAWN)) {
                        epValid = true;
                        break;
                    }
                }
    ...
    public static final String toFEN(Position pos) {
        ...
        ret.append(pos.isWhiteTurn() ? " w " : " b ");
        ...
        // Move counters
        ret.append(' ');
        ret.append(pos.getHalfMoveClock());
        ret.append(' ');
        ret.append(pos.getFullMoveCounter());
    ...
    private static final String moveToString(Position pos, Move move, boolean longForm, MoveGen.MoveList moves) {
        ...
        if (ret.length() == 0) {
            ...
            if (longForm) {
                ...
                if (p == (pos.isWhiteTurn() ? Piece.WPAWN : Piece.BPAWN)) {
    ...
    private static final boolean isCapture(Position pos, Move move) {
        if (pos.getPiece(move.to) == Piece.EMPTY) {
            int p = pos.getPiece(move.from);
            if ((p == (pos.isWhiteTurn() ? Piece.WPAWN : Piece.BPAWN)) && (move.to == pos.getEpSquare())) {
                return true;
            } else {
                return false;
            }
