...
import br.unicamp.ic.sed.engine.impl.TranspositionTable.TTEntry;
    ...
    final public int negaScout(int alpha, int beta, int ply, int depth, int recaptureSquare,
        ...
        if (    (depth >= 3*plyScale) && !inCheck && sti.allowNullMove &&
            ...
            if (nullOk) {
                ...
                pos.setWhiteMove(!pos.isWhiteTurn());
        ...
        for (int mi = 0; mi < moves.size; mi++) {
            ...
            Move m = moves.m[mi];
            if (pos.getPiece(m.to) == (pos.isWhiteTurn() ? Piece.BKING : Piece.WKING)) {
                moveGen.returnMoveList(moves);
                int score = MATE0-ply;
                if (log != null) log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_EXACT, evalScore, hKey);
            ...
            if (doFutility) {
                ...
                if (isCapture && (givesCheck || (depth + extend) > plyScale)) {
                    ...
                    int fVal = Piece.pieceValue[pos.getPiece(m.from)];
                    int tVal = Piece.pieceValue[pos.getPiece(m.to)];
                    final int pV = Piece.pV;
...
/*              if (ply <= 3) {
    ...
    private final boolean weakPlaySkipMove(Position pos, Move m, int ply) {
        long rndL = pos.zobristHash() ^ Position.getPshashkeys()[0][m.from] ^
                    Position.getPshashkeys()[0][m.to] ^ randomSeed;
        double rnd = ((rndL & 0x7fffffffffffffffL) % 1000000000) / 1e9;

        double s = strength * 1e-3;
        double offs = (17 - 50 * s) / 3;
        double effPly = ply * Evaluate.interpolate(pos.wMtrl + pos.bMtrl, 0, 30, Piece.qV * 4, 100) * 1e-2;
        ...
        for (int mi = 0; mi < moves.size; mi++) {
            ...
            if (inCheck) {
                ...
                if ((pos.getPiece(m.to) == Piece.EMPTY) && (m.promoteTo == Piece.EMPTY)) {
                    ...
                    if (negSEE(m))
                        continue;
                    int capt = Piece.pieceValue[pos.getPiece(m.to)];
                    int prom = Piece.pieceValue[m.promoteTo];
    ...
    final public int signSEE(Move m) {
        int p0 = Piece.pieceValue[pos.getPiece(m.from)];
        int p1 = Piece.pieceValue[pos.getPiece(m.to)];
        if (p0 < p1)
            return 1;
        return SEE(m);
    }
    ...
    final public int SEE(Move m) {
        final int kV = Piece.kV;
        
        final int square = m.to;
        if (square == pos.getEpSquare()) {
            captures[0] = Piece.pV;
        } else {
            captures[0] = Piece.pieceValue[pos.getPiece(square)];
            if (captures[0] == kV)
                return kV;
        }
        ...
        boolean white = pos.isWhiteTurn();
        int valOnSquare = Piece.pieceValue[pos.getPiece(square)];
    ...
    final void scoreMoveList(MoveGen.MoveList moves, int ply, int startIdx) {
        for (int i = startIdx; i < moves.size; i++) {
            ...
            if (isCapture) {
                ...
                score = Piece.pieceValue[v]/10 * 1000 - Piece.pieceValue[a]/10;
                if (seeScore > 0)
                    score += 2000000;
                else if (seeScore == 0)
                    score += 1000000;
    ...
    private final void scoreMoveListMvvLva(MoveGen.MoveList moves) {
        for (int i = 0; i < moves.size; i++) {
            ...
            int v = pos.getPiece(m.to);
            int a = pos.getPiece(m.from);
            m.score = Piece.pieceValue[v] * 10000 - Piece.pieceValue[a];
        }
    }
