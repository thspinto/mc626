...
//                	Log.w("debug", "NOT in movement x" + xCrd + sqSize / 2 + "y" + yCrd + sqSize / 2);
        ...
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                ...
                if (isUnderMovement() && selectedSquare == sq) {
                	Log.w("debug", "under movement x" + motionEvent.getX() + "y" + motionEvent.getY());
                	drawPieceOffset(canvas, (int) (motionEvent.getX() - sqSize /2 ), (int) ( motionEvent.getY() - sqSize /2), p, 10);
                } else {
                	Log.w("debug", "NOT in movement x" + xCrd + sqSize / 2 + "y" + yCrd + sqSize / 2);
                	drawPiece(canvas, xCrd + sqSize / 2, yCrd + sqSize / 2, p);
                }
...
//    private final void drawPieceOffset(Canvas canvas, int xCrd, int yCrd, int p, int offset) {
//        String ps;
//        switch (p) {
//            case Piece.EMPTY:
//                ps = "";
...
//            if (ps.equals("m")) {
//            	canvas.drawBitmap(BitmapFactory.decodeResource(getResources(), chess.spl.R.drawable.bishop_black),
//            			xCrd - xCent , yCrd + yCent , null);
//            } else {
//            	canvas.drawText(ps, xCrd - xCent, yCrd - yCent, paint);
//            }
//
//            Log.w("debug", "In movement drawing x" + (xCrd - xCent) + "y" + (yCrd - yCent) );
//        }
//    }
...
//            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
		// note that this callback is needed to allow
		// cb.setOnLongClickListener(new View.OnLongClickListener())
		// on the MyCuckooChessActivity.java
